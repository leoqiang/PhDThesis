...
\todo{Check and add all abbreviations and symbols}
...
Most of these compendia center on eukaryotic organisms; only M$^{\textrm{3D}}$ 
...
\todo{add some lines to refer to the back-end system for more details}
...
expression changes; see Supplementary Text S1 for more details \todo{include 
supplementary information}). Starting from only condition contrasts, COLOMBOS 
...
log-ratio (for details see Supplementary Text S1 \todo{include supplementary}). 
...
\todo{Update the data to version 2}
\begin{table}
...
\section{Conclusions and future directions}\todo{Move future directions into 
the last chapter}
...
\section{Acknowledgements}\todo{move to preface}
